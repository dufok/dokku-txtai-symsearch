...
try:
    with engine.connect() as conn:
        ...
        specialized_tables = conn.execute(
            text(
                """
            SELECT table_name 
            FROM information_schema.tables 
...
def info():
    ...
    try:
        ...
        with engine.connect() as conn:
            bodies_count = (
                conn.execute(text("SELECT COUNT(*) FROM bodies")).scalar() or 0
            )
            titles_count = (
                conn.execute(text("SELECT COUNT(*) FROM titles")).scalar() or 0
            )
            authors_count = (
                conn.execute(text("SELECT COUNT(*) FROM authors")).scalar() or 0
            )
            ...
            bodies_sample = [
                ...
                for row in conn.execute(
                    text("SELECT id FROM bodies ORDER BY id LIMIT 3")
                )
            ]
            titles_sample = [
                row[0]
                for row in conn.execute(
                    text("SELECT id FROM titles ORDER BY id LIMIT 3")
                )
            ]
            authors_sample = [
                row[0]
                for row in conn.execute(
                    text("SELECT id FROM authors ORDER BY id LIMIT 3")
                )
            ]
        ...
        return {
            ...
            "configs": {
                ...
                "title": {"search_type": "fuzzy match (trigram)", "table": "titles"},
                "author": {
                    "model": MODEL_PATH,
...
def index_title(doc: TitleDocument):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            conn.execute(stmt, {"id": doc.id, "title": doc.title})
            conn.commit()
...
def search_combined(req: SearchRequest):
    ...
    Only falls back to body matches if not enough title matches are found.
    """
    try:
        ...
        with engine.connect() as conn:
            ...
            exact_matches = conn.execute(
                exact_match_stmt, {"query": req.text, "query_lower": query_text}
            )
                ...
                title_results.append(
                    {
                        "id": row.id,
                        "title": row.title,
                        "score": float(row.base_score) * 2.5,
                        ...
                        title_results.append(
                            {
                                "id": row.id,
                                "title": row.title,
                                "score": float(row.base_score) * 1.8,
            ...
            if len(title_results) >= req.limit:
                title_results.sort(key=lambda x: x["score"], reverse=True)
                return {"results": title_results[: req.limit]}

            # Otherwise, fill with body matches
            body_stmt = text(
                """
            ...
            body_matches = conn.execute(
                body_stmt, {"query_embedding": query_embedding, "limit": req.limit * 2}
            )
            body_results = []
...
def search_author(req: SearchRequest):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # Process results
            rows = [
                {"id": row.id, "name": row.name, "score": float(row.combined_score)}
                for row in results
            ]
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if "bodies" in existing_tables:
                results["bodies_count"] = conn.execute(
                    text("SELECT COUNT(*) FROM bodies")
                ).scalar()

            if "titles" in existing_tables:
                results["titles_count"] = conn.execute(
                    text("SELECT COUNT(*) FROM titles")
                ).scalar()

            if "authors" in existing_tables:
                results["authors_count"] = conn.execute(
...
def health_check():
    ...
    try:
        ...
        try:
            ...
            models_ok = (
                body_vector is not None
                and len(body_vector) > 0
                and author_vector is not None
                and len(author_vector) > 0
            )
        ...
        return {
            "status": (
                "healthy" if (db_ok and models_ok and tables_exist) else "unhealthy"
            ),
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            bodies_sample = [
                row[0]
                for row in conn.execute(
                    text("SELECT id FROM bodies ORDER BY id LIMIT 5")
                )
            ]
            titles_sample = [
                row[0]
                for row in conn.execute(
                    text("SELECT id FROM titles ORDER BY id LIMIT 5")
                )
            ]
            authors_sample = [
                row[0]
                for row in conn.execute(
                    text("SELECT id FROM authors ORDER BY id LIMIT 5")
                )
            ]
        ...
        return {
            "status": (
                "healthy"
                if not (null_bodies_ids or null_authors_ids)
                else "inconsistent"
            ),
            "counts": {
                "bodies": bodies_count,
            ...
            "consistency": {
                "status": (
                    "ok" if not (null_bodies_ids or null_authors_ids) else "issues"
                ),
                "null_embeddings": {
...
def delete_all_data():
    ...
    try:
        ...
        print(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"Failed to erase all data: {str(e)}"
        )
